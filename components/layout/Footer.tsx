        ...
        <div className="container py-12">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              ...
              <p className="text-slate-300 mt-1 font-body">
            ...
            <div className="flex gap-3 flex-shrink-0">
              <Button variant="cta" asChild>
                <Link href="/profitability-health-check">
                  Free Profitability Review <ArrowRight className="w-4 h-4" />
                </Link>
              </Button>
      ...
      <div className="container py-16">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="space-y-2 text-sm text-slate-400 font-body">
              <div className="flex items-center gap-2">
                <MapPin className="w-4 h-4 text-brand-teal-400" />
                <span>Melbourne, VIC, Australia</span>
              </div>
              <div className="flex items-center gap-2">
                <Mail className="w-4 h-4 text-brand-teal-400" />
                <a href="mailto:admin@marginfy.com" className="hover:text-white transition-colors">
                  admin@marginfy.com
                </a>
              </div>
            </div>
        ...
        <div className="mt-12 pt-8 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-slate-500 text-sm font-body">
            © {new Date().getFullYear()} Marginfy. All rights reserved.
          </p>
          <p className="text-slate-500 text-sm font-body">
            Fractional CFO Services · Melbourne, Australia · Australia-wide
          </p>
        </div>
      </div>
    </footer>
...
function FooterColumn({ title, links }: { title: string; links: { label: string; href: string }[] }) {
  return (
    <div>
      <h4 className="text-xs font-bold text-white uppercase tracking-widest mb-4 font-heading">
        {title}
      </h4>
      <ul className="space-y-2.5">
        {links.map((link) => (
          <li key={link.href}>
            <Link
              href={link.href}
              className="text-sm text-slate-400 hover:text-brand-teal-400 transition-colors font-body"
            >
              {link.label}
            </Link>
          </li>
