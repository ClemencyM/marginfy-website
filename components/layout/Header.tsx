    ...
    <header className={cn(
      ...
      <nav className="container mx-auto flex items-center justify-between h-16 lg:h-18">
        ...
        <Link href="/" className="flex items-center flex-shrink-0">
          {scrolled ? (
            <Image
              src="/images/marginfy-logo-original.png"
              alt="Marginfy"
              width={140}
              height={34}
            ...
            <Image
              src="/images/marginfy-logo-white.png"
              alt="Marginfy"
              width={140}
              height={34}
              className="h-8 w-auto"
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          <Button variant="cta" size="sm" asChild>
            <Link href="/profitability-health-check">
              Free Health Check <ArrowRight className="w-3.5 h-3.5" />
            </Link>
          </Button>
        </div>
            ...
            <div className="container py-4 space-y-1">
              ...
              <div className="pt-4 space-y-2 border-t border-slate-100">
                ...
                <Button className="w-full" variant="cta" asChild>
                  <Link href="/profitability-health-check">Free Profitability Health Check</Link>
                </Button>
              </div>
            </div>
          </motion.div>
...
function NavLink({ href, children, scrolled }: {
  href: string; children: React.ReactNode; scrolled: boolean;
}) {
  return (
    <Link
      href={href}
...
function NavDropdown({ label, items, scrolled, isOpen, onToggle, onClose }: {
  label: string;
  ...
  return (
    <div className="relative" onMouseLeave={onClose}>
      <button
        className={cn(
          "flex items-center gap-1 px-3 py-2 text-sm font-medium rounded-lg transition-colors font-body",
          scrolled
            ? "text-slate-600 hover:text-brand-blue-500 hover:bg-slate-50"
            : "text-white/90 hover:text-white hover:bg-white/10"
        )}
          ...
          >
            {items.map((item) => (
              <Link
                ...
                onClick={onClose}
              >
                <div className="text-sm font-semibold text-slate-800 group-hover:text-brand-blue-500 font-heading">
                  {item.label}
                </div>
                {item.description && (
...
function MobileSection({ title, items }: { title: string; items: { label: string; href: string }[] }) {
  const [open, setOpen] = useState(false);
  return (
    <div>
      <button
        className="w-full flex items-center justify-between px-3 py-2.5 text-sm font-semibold text-slate-700 hover:text-brand-blue-500 font-heading"
          ...
          >
            {items.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="block px-3 py-2 text-sm text-slate-600 hover:text-brand-blue-500 font-body"
              >
                {item.label}
              </Link>
            ))}
...
function MobileSingleLink({ href, label }: { href: string; label: string }) {
  return (
    <Link
      href={href}
      className="block px-3 py-2.5 text-sm font-semibold text-slate-700 hover:text-brand-blue-500 font-heading"
    >
      {label}
    </Link>
  );
